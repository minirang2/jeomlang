...
const {
encodeString, encodeNumber, decodeString, decodeNumber,
tokenize, parse, JeomVM, JeomError, OP_TABLE, VERSION,
} = JeomEngine;
...
iface.once(‘line’, line => { iface.close(); resolve(line); });
...
makeDir:    async (dirPath)  => fs.mkdirSync(dirPath, { recursive: true }),
execCmd: async (cmd) => {
...
const vm = makeNodeVM({ searchPaths: [dir, ‘.’, …(opts.include || [])] });
try {
await vm.run(source);
} catch (e) {
...
if (!fs.existsSync(filePath)) {
console.error(red(`파일 없음: ${filePath}`)); process.exit(1);
}
const source = fs.readFileSync(filePath, ‘utf8’);
try {
...
console.error(red(`렉서 오류: ${e.message}`)); process.exit(1);
}
}
...
console.error(red(`오류: ${e.message}`)); process.exit(1);
}
}

function printNode(node, depth) {
const indent = ’  ‘.repeat(depth);
if (!node) { console.log(`${indent}${gray('null')}`); return; }
if (Array.isArray(node)) { node.forEach(n => printNode(n, depth)); return; }
const t = node.type;
...
} else if (t === ‘PUSH’)    { console.log(`${indent}${cyan('PUSH')}`); printNode(node.expr, depth+1); }
else if (t === ‘VARSET’)    { console.log(`${indent}${cyan('VAR')} ${yellow(node.name)} =`); printNode(node.expr, depth+1); }
else if (t === ‘GETVAR’)    { console.log(`${indent}${cyan('GET')} ${yellow(node.name)}`); }
else if (t === ‘FUNCDEF’)   { console.log(`${indent}${cyan('FUNC')} ${yellow(node.name)} ${gray(`(${node.args.join(’, ’)})`)}`);}
else if (t === ‘CALL’)      { console.log(`${indent}${cyan('CALL')} ${yellow(node.name)}`); }
else if (t === ‘INSTR’)     { console.log(`${indent}${magenta(node.op)}`); }
else if (t === ‘MAIN’)      { console.log(`${indent}${bold(cyan('MAIN'))}`); printNode(node.body, depth+1); }
else                        { console.log(`${indent}${cyan(t)}`); }
}

// ── REPL ──────────────────────────────────────────────────────────────────────
async function cmdRepl() {
console.log(` ${bold(cyan('점(Jeom)'))} ${gray(`v${VERSION}`)} REPL
${gray(‘───────────────────────────────────────’)}
...
const ast   = parse(toks);
// 새 함수 등록
ast.forEach(n => { if (n.type === ‘FUNCDEF’) vm._registerFunc(n); });
...
if (ans.toLowerCase() !== ‘y’) { console.log(gray(‘취소됨’)); return; }
}
const template =
...
··
⋮⋮
`; fs.writeFileSync(filePath, template, 'utf8'); console.log(green(`✓ 생성됨: ${filePath}`));
}
...
console.log(` ${bold(cyan('점(Jeom) 언어 CLI'))} ${gray(`v${VERSION}`)}
...
switch (cmd) {
case ‘run’: {
if (!args[1]) { console.error(red(‘파일 경로가 필요합니다’)); process.exit(1); }
const include = [];
let i = 2;
while (i < args.length) {
if (args[i] === ‘-I’ || args[i] === ‘–include’) { include.push(args[++i]); }
i++;
}
await cmdRun(args[1], { include });
break;
}
case ‘check’:
if (!args[1]) { console.error(red(‘파일 경로가 필요합니다’)); process.exit(1); }
...
break;
case ‘encode’: {
if (!args[1]) { console.error(red(‘문자열이 필요합니다’)); process.exit(1); }
...
if (isNaN(n)) { console.error(red(‘정수가 필요합니다’)); process.exit(1); }
...
break;
case ‘new’:
if (!args[1]) { console.error(red(‘파일 이름이 필요합니다’)); process.exit(1); }
cmdNew(args[1]);
break;
case ‘ops’:
cmdOps();
