/* jeom_ide.js — 점(Jeom) 웹 IDE 로직
 * jeom_engine.js 를 먼저 로드한 후 이 파일을 로드하세요.
 * 전역: JeomEngine (jeom_engine.js 에서 노출)
 */
(function () {
  'use strict';
  ...
  function openFile(e) {
    ...
    r.onload = ev => { editor.value = ev.target.result; updateLN(); };
  ...
  function updateLN() {
    ...
    document.getElementById('sChars').textContent = editor.value.length;
  }

  function updateCursor() {
    const p = editor.selectionStart, v = editor.value;
    const before = v.substring(0, p);
    document.getElementById('sLn').textContent = before.split('\n').length;
    document.getElementById('sCol').textContent = p - before.lastIndexOf('\n');
  }
